...
void InitGpioForSinglePhaseMode(void);
void InitEPwm1Example(void);
void InitEPwm2Example(void);
void InitEPwm3Example(void);
void InitEPwm3phInterleaved(void);
void InitEPwmMagicModulation(void);
...
#define DT .5 //2us
#define PWM_SWITCHING_FREQUENCY 10 //kHz
...
void InitEPwmMagicModulation(void)
{
    ...
    EPwm1Regs.TBPRD = TBPRD_TBCLKs; // Period = TBCLK counts
    EPwm1Regs.CMPA.half.CMPA = DT_TBCLKs; // adjust duty for output EPWM1A
    EPwm1Regs.CMPB = DUTY_TBCLKs; // adjust duty for output EPWM1A
    ...
    EPwm2Regs.TBPRD = TBPRD_TBCLKs; // Period = 1401 TBCLK counts
    EPwm1Regs.CMPA.half.CMPA = DT_TBCLKs;
    EPwm2Regs.CMPB = DUTY_TBCLKs; // adjust duty for output EPWM2A
    ...
    EPwm3Regs.TBPRD = TBPRD_TBCLKs; // Period = 801 TBCLK counts
    EPwm1Regs.CMPA.half.CMPA = DT_TBCLKs;
    EPwm3Regs.CMPB = TBPRD_TBCLKs-(2*DUTY_TBCLKs); // adjust duty for output EPWM3A
